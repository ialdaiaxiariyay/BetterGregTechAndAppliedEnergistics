...
public class MEInputDualConfigWidget extends Widget<MEInputDualConfigWidget>
    ...
    private final Supplier<Boolean> autoPullSupplier;
    ...
    public MEInputDualConfigWidget(IConfigurableSlotList slotList, int slotCount, boolean isFluid, String actionPrefix,
                                   Supplier<Boolean> autoPullSupplier) {
        this.slotList = slotList;
        this.slotCount = slotCount;
        this.isFluid = isFluid;
        this.autoPullSupplier = autoPullSupplier;
        this.columns = 8;
        this.actionPrefix = actionPrefix;
    }

    public MEInputDualConfigWidget withPrefix(PanelSyncManager syncManager) {
        this.syncManager = syncManager;
        this.configSyncHandler = new AEConfigSyncHandler(slotList, slotCount);
        syncManager.syncValue(actionPrefix + "_display", configSyncHandler);
        return this;
    }
    ...
    private void confirmAmountEdit() {
        ...
        if (amount > 0) {
            ...
            syncManager.callSyncedAction(actionPrefix + "_amount", buf -> {
    ...
    private boolean isAutoPull() {
        return autoPullSupplier != null && autoPullSupplier.get();
    }

    private boolean isStocking() {
        return (slotList instanceof ExportOnlyAEItemList && ((ExportOnlyAEItemList) slotList).isStocking()) ||
                (slotList instanceof ExportOnlyAEFluidList && ((ExportOnlyAEFluidList) slotList).isStocking());
    }
    ...
    public @NotNull Result onMousePressed(int button) {
        ...
        if (overConfig && !isAutoPull() && syncManager != null) {
            if (button == 1) {
                syncManager.callSyncedAction(actionPrefix + "_clear", buf -> buf.writeVarInt(slotIndex));
                return Result.SUCCESS;
            } else if (button == 0) {
                GenericStack config = configSyncHandler != null ? configSyncHandler.getClientConfig(slotIndex) : null;
                boolean holdingItem = false;
                if (Minecraft.getInstance().player != null) {
                    holdingItem = !Minecraft.getInstance().player.containerMenu.getCarried().isEmpty();
                }
                if (config != null && !holdingItem) {
                    openAmountEditor(slotIndex);
                } else {
                    syncManager.callSyncedAction(actionPrefix + "_set", buf -> buf.writeVarInt(slotIndex));
                }
                return Result.SUCCESS;
            }
        }
        ...
        if (overStock && !isStocking() && !isFluid && button == 0 && syncManager != null) {
            syncManager.callSyncedAction(actionPrefix + "_stock_pickup", buf -> buf.writeVarInt(slotIndex));
            return Result.SUCCESS;
        }
    ...
    public void setGhostIngredient(@NotNull ItemStack ingredient) {
        ...
        if (isFluid) {
            FluidUtil.getFluidContained(ingredient).ifPresent(fluid -> {
                if (!fluid.isEmpty()) {
                    syncManager.callSyncedAction(actionPrefix + "_set_ghost", buf -> {
